...
if save["first timer?"] == True:
    ...
    print(
        "Oh hey! You're back. If you forgot anything at all, type 'help'. Otherwise, just keep going!\n\n"
    )
...
index_to_attribute = {
    "1": "six hours of 1.5x jump height",
...
def shop():
    ...
    print(
        "Thanks for visiting the shop. Now go back to grinding for points.\n\n"
    )
...
def shopcommand(buy_check):
    if buy_check == "checkname":
        thing_to_check = constrained_input(
            "What do you need to check the name of? (number 1-10)\n\n",
            is_shop_valid)
        print(f"That is the {index_to_item[thing_to_check]}.")
    if buy_check == "checkprice":
        thing_to_check = constrained_input(
            "What do you need to check the price of? (number 1-10)\n\n",
            is_shop_valid)
        print(f"That item costs {index_to_price[thing_to_check]} tickets.")
    if buy_check == "checkattribute":
        thing_to_check = constrained_input(
            "What do you need to know the purpose of? (number 1-10)\n\n",
            is_shop_valid)
        print(f"That item gives you {index_to_attribute[thing_to_check]}.")
    if buy_check == "checkpurchasability":
        thing_to_check = constrained_input(
            "What do you need to know the purchasability of? (number 1-10)\n\n",
            is_shop_valid)
        if bcdict[thing_to_check] == "--":
            print("You have already acquired all purchases of this item.")
        print(
            f"That item's purchases {index_to_purchasability[thing_to_check]}."
        )

    while buy_check == "1":
        item = constrained_input(
            "Type the number of the item you would like to buy.\n\n",
            is_shop_valid)
        if item in ["4", "5", "7", "8"]:
            if bcdict[item] == "":
                bcdict[item] = "--"
                ...
                print(
                    "You have already purchased this one-time purchase item.\n\n"
                )
        elif item == "6":
            areyousure = constrained_input(
                """This is a really, really stupid decision. This will do you nothing good.
            ...
            if bcdict[item] == "--":
                print(
                    "You have run out of purchases for this two-time purchase item.\n\n"
                )
            else:
                bcdict[item] += "-"
                buy_item(item)
...
def roulette():
    ...
    bet = int(
        constrained_input(
            "How many points do you want to bet? Type '0' to exit the casino.\n\n",
            isvalidbet))
    if bet == 0:
        return
    points -= bet

    gametype = constrained_input(
        r"""
...
""", is_std_valid)
    ...
    if gametype == "1":
        color = constrained_input(
            "Would you like to bet on red or black? Respond with '1' for red, and '0' for black.\n\n",
            is_std_valid)
        ...
        if win:
            ...
            time.sleep(5)
            print(
                f"""Congratulations! You won the bet. Your {bet} points have been converted into {bet // 4} tickets."""
            )
        else:
            tickets += bet // 20
...
Your {bet} points have been converted into {bet // 20} tickets.""")
        ...
        if win:
            ...
            total_tickets += bet
            print(
                f"""Congratulations! You won the bet. Your {bet} points have all been converted into tickets."""
            )
        else:
            num_options = list(range(37))
            num_options.remove(int(number))
            print(
                f"""Unfortunately, the wheel landed on {random.choice(num_options)}.
...
def blackjack():
    ...
    bet = int(
        constrained_input(
            r"""How many points do you want to bet? Type '0' to exit the casino.
...
If you want to hit, type 1. If you want to stay, type 0.\n\n""", is_std_valid)
    while decision == "1":
        ...
        player += player_hit
        print(
            f"""You hit and got a {player_hit}. You now have {player} in total."""
        )
        if player > 21:
            print(
                f"You busted. You lose. Your {bet} points have been converted into {bet // 20} tickets."
            )
            tickets += bet // 20
            total_tickets += bet // 20
            return
...
He now has a hand of {dealer}.""")
        if dealer > 21:
            print(
                f"The dealer busted. You win. Your {bet} points have been converted into {bet // 4} tickets."
            )
            tickets += bet // 4
            total_tickets += bet // 4
            return
    ...
    if dealer > player:
        print(
            f"The dealer wins. Your {bet} points have been converted into {bet // 20} tickets."
        )
        tickets += bet // 20
        total_tickets += bet // 20
    else:
        print(
            f"You win. Your {bet} points have been converted into {bet // 4} tickets."
        )
        tickets += bet // 4
        total_tickets += bet // 4
...
def use_item():
    item = constrained_input(
        """Input the shop index of the item you would like to use.
...
def details(advanced):
    ...
    if advanced == "1":
        ...
        print(
            """You will also get an emoji. You must respond with the likelihood of that emoji being accurate.
        ...
        print(
            """If you would like to be more optimistic, or more pessimistic, you will have to reflect that over ALL messages. For example:
        ...
        print(
            """Since you are doing the survey on advanced mode, you will receive 10 points per question complete!
        ...
        print(
            """You will also get an emoji. You must respond with if you believe that that emoji is accurate.
        ...
        print(
            """Since you are doing the survey on standard mode, you will only receive 5 points per question complete.
        To switch modes at any point, type 'switch' and follow the instructions.\n\n"""
        )
...
def switch():
    global advanced
    print(
        """This menu is for if you would like to switch between standard and advanced mode.
    ...
    advanced = constrained_input(
        """Please choose if you would like to switch to advanced mode or standard mode.
    ...
    if advanced == "1":
        advanced = constrained_input(
            """Are you sure you would like to switch to advanced mode?\n\n""",
            is_std_valid)
        if advanced == "0":
            print(
                "Alright. You have been set to standard mode. If you change your mind, just type 'switch' again."
            )
    else:
        check = constrained_input(
            """Are you sure you would like to switch to standard mode?\n\n""",
            is_std_valid)
        if check == "0":
            print(
                "Alright. You have been set to advanced mode. If you change your mind, just type 'switch' again."
            )
            advanced = "1"
...
def save_and_exit(
    param1=None,
    param2=None
):  # these params will be passed by signal for no good reason, so gotta accept them
    save = {
        "it": items,
        "p": points,
        "tp": total_points,
    ...
    commands = ['git add .', 'git commit -m "Auto save commit"', 'git push']
...
if sys.platform == "win32":
    ...
    handler = ctypes.WINFUNCTYPE(ctypes.c_bool,
                                 ctypes.c_uint)(console_close_handler)
...
def is_shopcommand_valid(s):
    return s in [
        "0", "1", "checkname", "checkprice", "checkattribute",
        "checkpurchasability"
    ]


advanced = constrained_input(
    """Would you like to play in advanced mode? You will get double points.
    ...
    if advanced == "1":
        ...
        if response:
            if advanced_emoji_index:  # if the prompt hasn't switched
                with open("advanced_mode_results.txt", "a",
                          encoding="utf-8") as f:
        ...
        if response:
            if standard_emoji_index:  # if the prompt hasn't switched
                with open("standard_mode_results.txt", "a",
                          encoding="utf-8") as f:
                    f.write(f",{str(response)}")
            else:
                with open("standard_mode_results.txt", "a",
